...
internal class OpenNgcApiResourceIT {
    ...
    fun `should load catalog when authenticated with correct role`() {
        justRun { openNGCService.loadCatalog() }
        ...
        verify { openNGCService.loadCatalog() }
    }

    @Test
    @TestSecurity(user = "someuser", roles = ["any-role"])
    fun `should not allow loading catalog without the correct role`() {
        justRun { openNGCService.loadCatalog() }
        ...
        verify(exactly = 0) { openNGCService.loadCatalog() }
    }
    ...
    @Test
    fun `should return 404 when no catalog file is found`() {
        every { openNGCService.getLastCatalogUpdate() } throws CatalogNotLoadedError("test exception")
